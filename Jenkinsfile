...
node('shiv') {
        ...
        stage('one') {
                ...
                sh " chmod -R 755 scripts && scripts/test.sh root@${wkcNode} && chmod -R 755 scripts ; scripts/test.sh ${wkcNodeLabel} && st=$? ;if [[ ${swarm_rc} = 1 ]] ; then exit 1 ; fi && scripts/test.sh ${wkcNodeLabel} "
